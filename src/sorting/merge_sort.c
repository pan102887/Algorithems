    ...
    sort_stats_t *stats)
{
    ...
    return internal_merge_sort(arr, 0, arr_len - 1, element_size, cmp, stats);
}
    ...
    sort_stats_t *stats)
{
    if (left < right)
    {
        ...
        sort_result_t res = internal_merge_sort(arr, left, mid, element_size, cmp, stats);
        INCRE_COMPARISONS(stats);
        if (res != SORT_SUCCESS)
            return res;

        res = internal_merge_sort(arr, mid + 1, right, element_size, cmp, stats);
        INCRE_COMPARISONS(stats);
        if (res != SORT_SUCCESS)
            return res;

        return merge(arr, left, mid, right, element_size, cmp, stats);
    }
    return SORT_SUCCESS;
}
    ...
    sort_stats_t *stats)
{
    ...
    INCRE_MOVEMENTS_BY(stats, size_l + size_r);
    INCRE_MEMORY_USED(stats, (size_l + size_r) * element_size);
    ...
    free(left_arr);
    free(right_arr);
    DECRE_MEMORY_USED(stats, (size_l + size_r) * element_size);
    return SORT_SUCCESS;
}
